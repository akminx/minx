...
class _StructuredPromptLLM:
    async def run_structured_prompt(
        self, prompt: str, result_model: type[GoalCaptureInterpretation]
    ):
        assert prompt == "test"
...
@pytest.mark.asyncio
async def test_run_interpretation_falls_back_to_json_prompt_when_structured_prompt_is_unsupported() -> (
    None
):
    result = await run_interpretation(
