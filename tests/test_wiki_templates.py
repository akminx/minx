...
from tests.helpers import MinxTestConfig, read_resource_text
...
def test_wiki_template_resources_include_memory_and_human_edit_sections(tmp_path: Path) -> None:
    ...
    names = json.loads(asyncio.run(read_resource_text(server, "wiki-templates://list")))
    ...
    memory = asyncio.run(read_resource_text(server, "wiki-templates://memory"))
